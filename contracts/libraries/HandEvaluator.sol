...
library HandEvaluator {
    ...
    function evaluate(uint8[] memory cards, uint8 count) internal pure returns (uint32 rank) {
        ...
        // Scan values from King (12) down to Ace (0) for natural ordering
        for (uint8 v = 13; v >= 1; v--) {
            uint8 vIdx = v - 1;
            ...
            if (v == 1) break; // prevent underflow on uint8
        }
    ...
    function _findStraight(uint16 mask) internal pure returns (bool, uint8) {
        // Check from high to low: AKQJT, KQJ10 9, ..., 5 4 3 2 A
        // Pattern: 5 consecutive bits set
        for (uint8 high = 12; high >= 4; high--) {
            uint16 pattern = uint16(0x1F << (high - 4));
        ...
        uint16 wheel = 0x100F; // bit 0,1,2,3 + bit 12
        if (mask & wheel == wheel) {
    ...
    function _highBit(uint16 mask) internal pure returns (uint8) {
        for (uint8 i = 12; i >= 1; i--) {
            if (mask & uint16(1 << i) != 0) return i + 1;
            if (i == 0) break;
        }
        if (mask & 1 != 0) return 1; // Ace low
        return 0;
    }
    ...
    function _highestExcluding(uint16 mask, uint8 excludeValue) internal pure returns (uint8) {
        uint16 cleaned = mask & ~uint16(1 << (excludeValue - 1));
    ...
    function _topNKickers(uint16 mask, uint8 n) internal pure returns (uint32 result) {
        uint8 found;
        for (uint8 i = 12; i < 255; i--) {
            if (mask & uint16(1 << i) != 0) {
                result |= uint32(i + 1) << ((n - 1 - found) * 4);
                found++;
                if (found == n) break;
            }
            if (i == 0) break;
