...
contract PokerGame is IVRFConsumer {
    ...
    function joinTable(uint256 tableId, uint256 buyIn) external tableExists(tableId) {
        ...
        uint8 seat = s.playerCount; s.playerCount++;
        PlayerState storage p = playerStates[tableId][msg.sender];
        p.addr = msg.sender; p.chips = buyIn; p.isActive = true; p.isSeated = true; p.seatIndex = seat;
    ...
    function leaveTable(uint256 tableId) external tableExists(tableId) onlySeated(tableId) {
        ...
        seatMap[tableId][p.seatIndex] = address(0);
    ...
    function evaluateShowdown(uint256 tableId) external tableExists(tableId) {
        ...
        address bestPlayer; uint32 bestRank;
        for (uint8 i = 0; i < s.playerCount; i++) {
            address addr = seatMap[tableId][i];
            if (addr == address(0)) continue;
            PlayerState storage p = playerStates[tableId][addr];
            ...
            uint8[7] memory hand;
            hand[0] = p.revealedCard0; hand[1] = p.revealedCard1;
            for (uint8 c = 0; c < 5; c++) hand[2 + c] = s.community[c];

            uint32 rank = HandEvaluator.evaluateBestHand(hand);
            p.handRank = rank;
            if (rank > bestRank) { bestRank = rank; bestPlayer = addr; }
        }

        uint256 payout = s.pot; s.pot = 0;
        playerStates[tableId][bestPlayer].chips += payout;
        PokerLib.GameStatus prev = s.status;
        s.status = PokerLib.GameStatus.Settled;
        _resetSalts(tableId);
        emit ShowdownResult(tableId, bestPlayer, bestRank, payout);
    ...
    function _advanceGame(uint256 tableId) internal {
        ...
        uint8 nx = (s.activePlayerIndex + 1) % s.playerCount;
        for (uint8 c = 0; c < s.playerCount; c++) {
            address a = seatMap[tableId][nx];
            if (a != address(0) && playerStates[tableId][a].isActive) { s.activePlayerIndex = nx; return; }
            nx = (nx + 1) % s.playerCount;
        }
    }
    ...
    function _advanceRound(uint256 tableId) internal {
        ...
        s.activePlayerIndex = (s.dealerIndex + 1) % s.playerCount; s.lastActionBlock = block.number;
