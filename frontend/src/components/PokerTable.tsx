 ...
 *   a clear warning + "Recover Table" helper (timeouts betting or settles showdown).
...
function decodePackedCard(encoded: number): { suit: number, value: number } | null {
  if (!encoded) return null
  const suit = encoded >> 4
  const value = encoded & 0x0f
  if (suit < 0 || suit > 3 || value < 1 || value > 13) return null
  return { suit, value }
}

function encodePackedCard(suit: number, value: number): number {
  if (suit < 0 || suit > 3 || value < 1 || value > 13) return 0
  return (suit << 4) | value
}
...
function Card({ encoded, size = 'normal', flipDelay = 0 }: {
  ...
  const decoded = decodePackedCard(encoded)
  if (!decoded) return <span style={size === 'large' ? st.cardBackLg : size === 'small' ? st.cardBackSm : st.cardBack}>?</span>
  const { suit, value } = decoded
  ...
  try {
    ...
    for (let suit = 0; suit < 4; suit++) {
      for (let value = 1; value <= 13; value++) {
        deck.push(encodePackedCard(suit, value))
      }
    }

    let hash = deckSeed
    for (let i = 1; i < 52; i++) {
      hash = keccak256(encodePacked(['bytes32', 'uint8'], [hash, i]))
...
export default function PokerTable({ tableId, tableName, bigBlind, onBack }: PokerTableProps) {
  ...
  const revealedCardContracts = playerAddrs.map(addr => ({
  ...
  const { data: revealedCardsData, refetch: refetchRevealedCards } = useReadContracts({
    contracts: revealedCardContracts as any,
    query: { refetchInterval: 2000, enabled: playerAddrs.length > 0 }
  })
  ...
  const allPlayersRaw: PState[] = playerAddrs.map((addr, i) => {
    ...
    return {
      ...
      revealedCard0: revealed ? encodePackedCard(Number(revealed[0]), Number(revealed[1])) : 0,
      revealedCard1: revealed ? encodePackedCard(Number(revealed[2]), Number(revealed[3])) : 0,
  ...
  const refreshAll = useCallback(() => {
    refetchSession(); refetchPlayers(); refetchStates(); refetchRevealedCards(); refetchBal()
  }, [refetchSession, refetchPlayers, refetchStates, refetchRevealedCards, refetchBal])
  ...
  const handleSettleLastStanding = useCallback(async () => {
    if (!sessionAccount) return
    setActionPending(true); setLocalError(null)
    try {
      await sWrite('settleLastStanding', [tableId], undefined, 300_000n)
      addLog('Last player standing settled')
    } catch (err: any) {
      console.error('[SETTLE-LAST]', err)
  ...
  const handleRecoverTable = useCallback(async () => {
    if (!sessionAccount) return
    setActionPending(true); setLocalError(null)
    try {
      if (status >= 2 && status <= 5) {
        await sWrite('forceTimeout', [tableId], undefined, 300_000n)
        addLog('Timeout forced for inactive player')
      } else if (status === 6 && activePlayers.length === 1) {
        await sWrite('settleLastStanding', [tableId], undefined, 300_000n)
        addLog('Last player standing settled')
      } else if (status === 6 && activePlayers.length >= 2 && activePlayers.every(p => p.hasRevealed)) {
        await sWrite('evaluateShowdown', [tableId], undefined, 800_000n)
        addLog('Showdown resolved')
      } else {
        throw new Error('No recovery action is available for the current table state yet.')
      }
    } catch (err: any) {
    ...
    setActionPending(false)
    setTimeout(refreshAll, 1500)
  }, [sWrite, tableId, addLog, refreshAll, sessionAccount, status, activePlayers])
  ...
  useEffect(() => {
    ...
    if (status === 6) {
      if (activePlayers.length === 1) {
        runAuto(handleSettleLastStanding)
        return
      }
      if (activePlayers.length >= 2 && activePlayers.every(p => p.hasRevealed)) {
  ...
  const winnerAddrs = useMemo(() => {
    ...
    const rankedPlayers = allPlayers.filter(p => p.handRank > 0)
    if (rankedPlayers.length > 0) {
      const bestRank = Math.max(...rankedPlayers.map(p => p.handRank))
      return new Set(rankedPlayers.filter(p => p.handRank === bestRank).map(p => p.addr.toLowerCase()))
    }
    ...
    <div style={st.root}>
      ...
      {showStuckWarning && (
        <div style={{ ...st.errBanner, background: 'rgba(232,192,126,0.08)', borderColor: 'rgba(232,192,126,0.3)', color: '#E8C07E' }}>
          A hand is in progress ({STATUS_LABELS[status]}). Wait for it to finish, or use Recover Table if the action timer/showdown looks stuck.
          <button onClick={handleRecoverTable} disabled={txBusy || !sessionAccount} style={{ ...st.btnHelper, marginLeft: '10px', color: '#E8C07E', borderColor: 'rgba(232,192,126,0.3)' }}>
            Recover Table
          </button>
        </div>
      )}
        ...
        <div style={st.tableFelt}>
          ...
          <div style={st.seatsContainer}>
            {Array.from({ length: 6 }, (_, seatIdx) => {
              ...
              const isWinner = winnerAddrs.has(player.addr.toLowerCase())
              ...
              return (
                <div key={seatIdx} style={{ ...st.seatWrap, ...pos, transform: 'translate(-50%,-50%)' }}>
                  ...
                  <div style={{
                    ...
                    {isWinner && <div style={st.winLabel}>{player.handRank ? `WINNER ${rankLabel(player.handRank)}` : 'WINNER'}</div>}
                  </div>
                </div>
              )
            })}
