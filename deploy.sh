...
do_verify() {
    ...
    echo ""
    echo "  # Deposit 10 INIT into your internal balance:"
    echo "  cast send --rpc-url ${RPC_URL} --private-key \$DEPLOYER_PRIVATE_KEY \\"
    echo "    ${POKER_GAME_ADDRESS} 'deposit()' --value 10ether"
    echo ""
    echo "  # Join table 0 with a 10 INIT buy-in:"
    echo "  cast send --rpc-url ${RPC_URL} --private-key \$DEPLOYER_PRIVATE_KEY \\"
    echo "    ${POKER_GAME_ADDRESS} 'joinTable(uint256,uint256)' 0 10ether"
    echo ""
...
cast send --rpc-url http://localhost:8545 --private-key \$KEY \\
  ${poker_addr} 'deposit()' --value 10ether

# Join table:
cast send --rpc-url http://localhost:8545 --private-key \$KEY \\
  ${poker_addr} 'joinTable(uint256,uint256)' 0 10ether

# Request deal (VRF + Fisher-Yates shuffle):
cast send --rpc-url http://localhost:8545 --private-key \$KEY \\
  ${poker_addr} 'requestDeal(uint256)' 0

# Check your table state:
cast call --rpc-url http://localhost:8545 \\
  ${poker_addr} 'getPlayerState(uint256,address)' 0 \$ADDR
\`\`\`
SECTION
    fi
